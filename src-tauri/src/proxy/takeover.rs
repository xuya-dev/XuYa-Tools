...
/// 恢复 Codex: 用备份覆盖 auth.json + config.toml
pub fn restore_codex(auth_backup: &Path, config_backup: &Path) -> TakeoverResult {
    if auth_backup.exists() {
        let r = restore_from_backup(auth_backup, &codex::codex_auth_path(), "Codex CLI auth");
        if !r.success {
            return r;
        }
    }
    if config_backup.exists() {
        let r = restore_from_backup(
            config_backup,
    ...
    TakeoverResult {
        success: true,
        message: "Codex CLI 已恢复".to_string(),
    }
}
