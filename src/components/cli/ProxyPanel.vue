...
const { proxyStatus, proxyBusy, refreshProxyStatus, startProxy, stopProxy, setTakeover } =
  useCliConfig();
...
const startedAtText = computed(() => {
  ...
  return new Date(proxyStatus.value.started_at * 1000).toLocaleTimeString('zh-CN', {
    hour12: false,
  });
});
...
.proxy-status-dot.on {
  ...
  box-shadow: 0 0 8px var(--xuya-success-soft);
}
.proxy-status-text {
  font-size: 12px;
  color: var(--xuya-text-2, #888);
}
.proxy-toggle-btn {
  padding: 7px 18px;
  font-size: 13px;
...
.proxy-toggle-btn:hover:not(:disabled) {
  filter: brightness(1.1);
}
.proxy-toggle-btn.running {
  background: var(--xuya-danger, #ef4444);
}
...
.proxy-detail {
  ...
  gap: 8px;
  padding: 12px 0;
  border-top: 1px solid var(--xuya-border, rgba(127, 127, 127, 0.12));
}
.proxy-detail-row {
  display: flex;
  align-items: center;
...
.slider {
  ...
  cursor: pointer;
  inset: 0;
  background: var(--xuya-border, #444);
  border-radius: 22px;
  transition: 0.2s;
}
.slider::before {
...
.mono {
  font-family: var(--xuya-font-mono, 'Consolas', monospace);
}
.cli-mini-btn {
  display: inline-flex;
  align-items: center;
  ...
  font-size: 11px;
  color: var(--xuya-text-2, #888);
  background: var(--xuya-bg-elevated, rgba(127, 127, 127, 0.1));
