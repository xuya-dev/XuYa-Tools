<template>
  <div class="cli-panel">
    ...
    <div class="cli-status-grid">
      <div
        class="cli-status-card"
        :class="{ active: claudeActiveProvider, 'proxy-on': claudeProxyOn }"
      >
        ...
        <div
          v-if="
            claudeModels.model_sonnet ||
            claudeModels.model_opus ||
            claudeModels.model_haiku ||
            claudeModels.model
          "
        ...
        >
          ...
          <span class="cli-status-val model-pills">
            <span v-if="claudeModels.model_sonnet" class="live-model-pill">
              Sonnet {{ claudeModels.sonnet_name || strip1M(claudeModels.model_sonnet) }}
              <span v-if="has1M(claudeModels.model_sonnet)" class="onem-badge">1M</span>
            </span>
            <span v-if="claudeModels.model_opus" class="live-model-pill">
              Opus {{ claudeModels.opus_name || strip1M(claudeModels.model_opus) }}
              <span v-if="has1M(claudeModels.model_opus)" class="onem-badge">1M</span>
            </span>
            <span v-if="claudeModels.model_haiku" class="live-model-pill">
              Haiku {{ claudeModels.haiku_name || strip1M(claudeModels.model_haiku) }}
            </span>
            <span
              v-if="
                !claudeModels.model_sonnet &&
                ...
                claudeModels.model
              "
              class="live-model-pill"
            >
              {{ strip1M(claudeModels.model) }}
              <span v-if="has1M(claudeModels.model)" class="onem-badge">1M</span>
            </span>
          </span>
        </div>
        <div class="cli-status-row">
          <span class="cli-status-label">配置路径</span>
          <span class="cli-status-val mono small">
            {{ status?.claude.config_path || '未检测到' }}
          </span>
        </div>
      </div>
      ...
      <div
        class="cli-status-card"
        :class="{ active: codexActiveProvider, 'proxy-on': codexProxyOn }"
      >
        <div class="cli-status-head">
          <div class="cli-app-name-wrap">
    ...
    <div class="config-viewer">
      <div class="config-viewer-head">
        ...
        <div v-if="configViewer.content" class="config-viewer-actions">
          <button
            ...
            :disabled="configViewer.loading"
            @click="loadConfigViewer(configViewer.app, true)"
          >
            <RefreshCw :size="13" />
            刷新
          </button>
      ...
      <div v-if="configViewer.open" class="config-viewer-body">
        ...
        <pre
          class="config-viewer-code"
        ><code v-if="configViewer.loading">加载中…</code><code v-else>{{ configViewer.content || '（无内容 / 文件不存在）' }}</code></pre>
      </div>
    </div>
    ...
    <div class="cli-providers">
      ...
      <div v-if="filteredProviders.length === 0" class="cli-empty">
        {{
          providerSearch
            ? `没有匹配 "${providerSearch}" 的 Provider`
      ...
      <div v-else class="provider-card-grid">
        <div
          ...
          :key="p.id"
          class="provider-card"
          :class="{ active: isCurrentForTab(p) }"
        >
          ...
          <div class="provider-card-head">
            ...
            <div class="provider-card-info">
              ...
              <span class="provider-card-cat" :class="p.category">
                {{ CATEGORY_LABELS[p.category] }}
                <span v-if="p.scope === 'both'" class="scope-badge">通用</span>
              </span>
            </div>
          </div>
          ...
          <div v-if="modelSummary(p).length" class="provider-models">
            <span
              v-for="(m, i) in modelSummary(p)"
              :key="i"
              class="model-tag"
              :class="m.role.toLowerCase()"
            >
              {{ m.role }}: {{ m.model }}
              <span v-if="m.oneM" class="model-1m-mark">·1M</span>
            </span>
          </div>
                ...
                >
                  <template v-if="item.isValid">
                    <div class="ba-header">
                      <span
                        class="ba-remaining"
                        :class="{ low: (item.remaining ?? 0) < (item.total ?? 0) * 0.1 }"
                      >
                    ...
                    <div v-if="item.total != null && item.total > 0" class="ba-bar">
                      <div
                        class="ba-bar-fill"
                        :style="{
                          width: Math.min(100, ((item.used ?? 0) / item.total) * 100) + '%',
                        }"
                  ...
                  >
                    <template v-if="item.isValid">
                      <div class="pt-header">
                        ...
                        <span class="pt-pct" :class="{ high: (item.used ?? 0) >= 80 }">
                          {{ item.used?.toFixed(0) }}%
                        </span>
                      </div>
                      <div class="pt-bar">
                      ...
                      <div v-if="item.resetsAt" class="pt-reset">
                        重置 {{ formatBalanceTime(item.resetsAt) }}
                      </div>
                    </template>
                    <div v-else class="ba-invalid">
                      <span class="ba-invalid-icon">!</span>
        ...
        <div class="cli-modal-card cli-modal-large">
          ...
          <div v-if="!editor.isEdit && editor.step === 'preset'" class="cli-modal-body">
            ...
            <div class="preset-header">
              ...
              <input v-model="presetSearch" class="preset-search" placeholder="搜索预设..." />
            </div>
          ...
          <div v-else class="cli-modal-body">
            ...
            <div v-if="!editor.isEdit && editor.form.preset_id" class="selected-preset-bar">
              <div
                class="provider-icon sm"
                :style="{ color: editor.form.icon_color || brandColor(editor.form.icon) }"
              >
                <span
                  v-if="hasIconImage(editor.form.icon)"
                  class="provider-icon-svg"
                  v-html="iconSvg(editor.form.icon)!"
                ></span>
                <span v-else>{{ iconText(editor.form.name) }}</span>
              </div>
              <span>{{ selectedPreset?.name || editor.form.name }}</span>
              <span class="selected-preset-cat" :class="editor.form.category">
                {{ CATEGORY_LABELS[editor.form.category] }}
              </span>
            </div>
            ...
            <div class="form-section">
              ...
              <label class="cli-field">
                <span>官网链接</span>
                <input
                  v-model="editor.form.website_url"
                  class="mono"
              ...
              <label class="cli-field">
                ...
                <div class="api-key-wrap">
                  ...
                  >
                    <Eye v-if="!apiKeyVisible" :size="14" />
              ...
              <label
                v-if="
                  editor.form.category !== 'official' && editor.form.category !== 'cloud_provider'
                "
                class="cli-field"
              >
                <span>API 端点地址</span>
                <input
                  v-model="editor.form.base_url"
                  class="mono"
            ...
            <div v-if="editor.appTab === 'claude'" class="form-section">
              <div class="form-section-title">
                ...
                <button
                  ...
                  :disabled="
                    modelsLoading || !editor.form.base_url || editor.form.category === 'official'
                  "
                  @click="onFetchModels"
                >
              ...
              <div class="model-role-grid">
                <div class="model-role-row">
                  ...
                  />
                  <label
                    class="model-1m-toggle"
                    title="声明支持 1M 上下文 (写入模型名 [1M] 后缀, Claude Code 原生识别)"
                  >
              ...
              <span class="field-hint">
                实际模型 = 发给上游的模型 ID（可勾选 1M 启用百万上下文）；显示名 = Claude Code
                <code>/model</code>
                菜单里的友好名称（可选）
              </span>
              ...
              <label class="cli-field">
                <span>默认兜底模型（可选）</span>
                <input
                  v-model="editor.form.model"
                  class="mono"
                  list="xuya-fetched-models"
                  placeholder="留空则按 Sonnet → Opus → Haiku 取主模型"
                />
                <span class="field-hint">
                  Claude Code 按角色分别使用模型 (ANTHROPIC_DEFAULT_*_MODEL)。勾选 1M
                  会给对应模型名追加 [1M] 后缀，启用百万 token 上下文窗口。
                </span>
              </label>
              <span v-if="fetchedModels.length" class="field-hint">
                已获取 {{ fetchedModels.length }} 个模型，可在输入框下拉选择
              </span>
            </div>
            ...
            <div v-if="editor.appTab === 'codex'" class="form-section">
              ...
              <label class="cli-field">
                ...
                <input
                  ...
                  class="mono"
                  list="xuya-fetched-models"
                  placeholder="如 gpt-4o、o3、deepseek-chat"
                />
            ...
            <div class="advanced-toggle" @click="advancedOpen = !advancedOpen">
              ...
              <span class="advanced-toggle-hint">
                认证字段、API 格式、User-Agent、原始配置文件、余额查询
              </span>
            </div>
            <div v-if="advancedOpen" class="advanced-section form-section">
              ...
              <label class="cli-field">
                ...
                <span class="field-hint">
                  {{
                    editor.appTab === 'codex'
                      ? 'Codex CLI 原生支持的格式直接直连,其他格式需启动代理转换'
                      : 'Claude Code 原生支持 Anthropic,其他格式需启动代理转换'
                  }}
                </span>
              </label>
              ...
              <label class="cli-field">
                ...
                <input
                  v-model="editor.form.custom_user_agent"
                  class="mono"
              ...
              <label v-if="editor.appTab === 'claude'" class="cli-field">
                <span>
                  <span class="file-badge">settings.json</span>
                  （高级，留空则自动生成）
                </span>
                <textarea
                ...
                ></textarea>
                <span class="field-hint">
                  上方为根据当前字段实时生成的预览。填入后完全覆盖自动生成的 Claude Code 配置
                </span>
              </label>
              ...
              <label v-if="editor.appTab === 'codex'" class="cli-field">
                <span>
                  <span class="file-badge">auth.json</span>
                  （高级，留空则自动生成）
                </span>
                <textarea
                ...
                ></textarea>
                <span class="field-hint">
                  上方为根据当前字段实时生成的预览。填入后完全覆盖自动生成的凭据
                </span>
              </label>
              <label v-if="editor.appTab === 'codex'" class="cli-field">
                <span>
                  <span class="file-badge">config.toml</span>
                  （高级，留空则自动生成）
                </span>
                <textarea
                ...
                ></textarea>
                <span class="field-hint">
                  上方为根据当前字段实时生成的预览。填入后完全覆盖自动生成的配置
                </span>
              </label>
              ...
              <template
                v-if="
                  editor.form.category === 'custom' && editor.form.quota_provider_type === 'newapi'
                "
              >
                <label class="cli-field">
                  <span>
...
import { RefreshCw, ExternalLink, Eye, EyeOff } from '@lucide/vue';
...
const filteredProviders = computed(() => {
  ...
  return byTab.filter((p) =>
    [p.name, p.note, p.base_url, p.website_url, p.model].some((f) => f?.toLowerCase().includes(kw)),
  );
});
...
function modelSummary(p: CliProvider): { role: string; model: string; oneM: boolean }[] {
  ...
  if (p.model_sonnet)
    out.push({
      role: 'Sonnet',
      model: p.sonnet_name || strip1M(p.model_sonnet),
      oneM: p.sonnet_1m || has1M(p.model_sonnet),
    });
  if (p.model_opus)
    out.push({
      role: 'Opus',
      model: p.opus_name || strip1M(p.model_opus),
      oneM: p.opus_1m || has1M(p.model_opus),
    });
  if (p.model_haiku)
    out.push({
      role: 'Haiku',
      model: p.haiku_name || strip1M(p.model_haiku),
      oneM: p.haiku_1m || has1M(p.model_haiku),
    });
  if (out.length === 0 && p.model)
    out.push({ role: '默认', model: strip1M(p.model), oneM: has1M(p.model) });
  return out;
}
...
async function onFetchModels() {
  ...
  try {
    ...
    if (msg.includes('HTTP 401') || msg.includes('HTTP 403')) {
      ...
      msg.includes('All candidates failed') ||
...
const claudeSettingsPreview = computed(() => {
  ...
  if (f.api_key) {
    const keyName =
      f.auth_field === 'ANTHROPIC_API_KEY' ? 'ANTHROPIC_API_KEY' : 'ANTHROPIC_AUTH_TOKEN';
...
const filteredPresets = computed(() => {
  ...
  const matched = kw
    ? list.filter((p) =>
        [p.name, p.id, p.baseUrl, p.websiteUrl].some((f) => f?.toLowerCase().includes(kw)),
      )
    : list;
  // 官方分类置顶
...
const onSwitch = async (app: AppType, id: string) => {
  ...
  if (p.category !== 'official' && p.api_format !== 'anthropic' && !isProxyOn && app === 'claude') {
...
async function autoQueryBalances() {
  const targets = providers.value.filter((p) => p.base_url && p.category !== 'official');
...
.cli-status-card.active {
  ...
  box-shadow:
    0 0 0 1px var(--xuya-success-soft),
    var(--xuya-shadow);
}
...
.cli-status-card.proxy-on {
  ...
  box-shadow:
    0 0 0 1px var(--xuya-accent-soft),
    var(--xuya-shadow);
}
...
.provider-card {
  ...
  gap: 12px;
  transition:
    transform var(--xuya-duration) var(--xuya-ease),
    box-shadow var(--xuya-duration) var(--xuya-ease),
    border-color var(--xuya-duration) var(--xuya-ease);
  overflow: hidden;
}
...
.provider-card.active::before {
  background: linear-gradient(
    180deg,
    var(--xuya-success),
    color-mix(in srgb, var(--xuya-success) 60%, #000)
  );
  opacity: 1;
}
